...
from tensorflow.compat.v1.nn import rnn_cell as rnn_cell_legacy
import numpy as np
...
class OnsetNetSergey4:
    def __init__(self,
        ...
        do_transformer = d_model > 0 and num_transformer_layers > 0 and num_heads > 0 and ff_dim > 0 
        do_dnn = len(dnn_sizes) > 0

        if not (do_rnn or do_transformer):
        ...
        if do_rnn:
            ...
            with tf.variable_scope('rnn_unroll'):
                rnn_inputs_seq = tf.reshape(rnn_inputs, [batch_size, rnn_nunroll, rnn_size])

                lstm_input = rnn_inputs_seq
                for i in range(rnn_nlayers):
                    lstm_layer = tf.keras.layers.Bidirectional(
                        tf.keras.layers.LSTM(
                            ...
                            name=f'bidir_lstm_{i}'
                        )
                    )
                    lstm_input = lstm_layer(lstm_input)

                outputs_seq = lstm_input
                rnn_output = tf.reshape(outputs_seq, [batch_size * rnn_nunroll, rnn_size * 2])
                rnn_output_size = rnn_size * 2

        print('rnn_output: {}'.format(rnn_output.get_shape()))
            
        if do_transformer:
            inputs_proj = tf.reshape(rnn_output, [batch_size, rnn_nunroll, rnn_output_size])

            # Make sure RNN output matches transformer input
            if rnn_output_size != d_model:
                with tf.variable_scope('transformer_proj'):
                    proj_w = tf.get_variable('W', [rnn_output_size, d_model], initializer=tf.keras.initializers.VarianceScaling(), dtype=dtype)
                    proj_b = tf.get_variable('b', [d_model], initializer=tf.constant_initializer(0.0), dtype=dtype)
                    inputs_proj = tf.nn.bias_add(tf.tensordot(inputs_proj, proj_w, axes=[[2], [0]]), proj_b)
                rnn_output_size = d_model
            
            pos_encoding = positional_encoding(rnn_nunroll, rnn_output_size)
            inputs_proj = inputs_proj + pos_encoding[:, :rnn_nunroll, :]
            ...
            for i in range(num_transformer_layers):
                attention_layers.append(
                    tf.keras.layers.MultiHeadAttention(
                        num_heads=num_heads,
                        key_dim=rnn_output_size // num_heads,
                        dropout=(1.0 - rnn_keep_prob) if mode == 'train' else 0.0
                    )
                )
                ffn_layers.append(
                    tf.keras.Sequential([
                        ...
                        tf.keras.layers.Dense(rnn_output_size)
                    ])
                )

            for i in range(num_transformer_layers):
                attn_output = attention_layers[i](inputs_proj, inputs_proj, training=(mode == 'train'))
            ...
            transformer_output = tf.reshape(inputs_proj, [batch_size * rnn_nunroll, rnn_output_size])
        ...
        self.do_rnn = do_rnn
        self.do_transformer = do_transformer
    ...
    def iterate_eval_batches(self, eval_chart, **feat_kwargs):
        ...
        for frame_idx in range(subseq_start, eval_chart.get_nframes(), subseq_len):
            ...
            if audio.shape[0] != subseq_len:
                print(f"[DEBUG] Skipping incomplete batch at frame {frame_idx}: got {audio.shape[0]} frames")
                continue
...
def positional_encoding(seq_len, d_model):
    ...
    angle_rads = angle_rads.astype(np.float32)
