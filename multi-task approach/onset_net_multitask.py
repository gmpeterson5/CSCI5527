...
#         print('feats_other: {}'.format(feats_other_nunroll.get_shape()))
        ...
        if mode != 'gen':
            ...
            # Add placeholders for beat tracking if multi-task is enabled
            if enable_multi_task:
                beat_targets_nunroll = tf.placeholder(dtype, shape=[batch_size, rnn_nunroll], name='beat_targets')
                beat_target_weights_nunroll = tf.ones([batch_size, rnn_nunroll], dtype, name='beat_target_weights')
        ...
        if mode == 'train' and input_keep_prob < 1.0:
            # We only apply dropout to the other features since audio features are handled by CNN
...
#                 print('{}: {}'.format(layer_name, pooled.get_shape()))
                ...
                # Apply CNN dropout after activation and pooling
                if mode == 'train' and cnn_keep_prob < 1.0:
                    pooled = tf.nn.dropout(pooled, cnn_keep_prob)
#                     print('{}_dropout: {}'.format(layer_name, pooled.get_shape()))
...
#         print('feats_all: {}'.format(feats_all.get_shape()))
        ...
        if do_rnn:
            ...
            if mode == 'train' and rnn_keep_prob < 1.0:
                # Enhanced RNN dropout - apply to both input and output
                cell = tf.nn.rnn_cell.DropoutWrapper(
                    cell, 
                    input_keep_prob=input_keep_prob,  # Apply dropout to inputs as well
            ...
            if rnn_nlayers > 1:
                ...
                for i in range(rnn_nlayers):
                    ...
                    if mode == 'train' and rnn_keep_prob < 1.0:
                        # Apply different dropout to each layer
                        # First and last layers often benefit from different dropout rates
                        if i == 0 or i == rnn_nlayers - 1:
                            layer_keep_prob = min(1.0, rnn_keep_prob + 0.1)  # Less dropout for first/last layers
                        else:
                            layer_keep_prob = rnn_keep_prob
                            
                        layer_cell = tf.nn.rnn_cell.DropoutWrapper(
                            layer_cell,
                            input_keep_prob=input_keep_prob if i == 0 else 1.0,  # Only apply input dropout to first layer
                            output_keep_prob=layer_keep_prob)
...
#             print('rnn_output: {}'.format(rnn_output.get_shape()))
            ...
            if do_attention:
                attention_weights = tf.get_variable('attention_w', [rnn_size, 1], initializer=tf.random_normal_initializer(stddev=0.1))
                attention_scores = tf.matmul(tf.reshape(outputs, [-1, rnn_size]), attention_weights)
                attention_scores = tf.reshape(attention_scores, [batch_size, rnn_nunroll])
                attention_weights = tf.nn.softmax(attention_scores, axis=1)
                attention_weights = tf.reshape(attention_weights, [batch_size, rnn_nunroll, 1])
                attention_context = tf.reduce_sum(tf.multiply(outputs, attention_weights), axis=1)
                rnn_output = attention_context
            else:
        ...
        if do_dnn:
            ...
            for i, layer_size in enumerate(dnn_sizes):
                ...
                if mode == 'train' and dnn_keep_prob < 1.0:
                    # Use slightly less aggressive dropout for the final layer
                    if i == len(dnn_sizes) - 1:
                        layer_keep_prob = min(1.0, dnn_keep_prob + 0.1)
                    else:
                        layer_keep_prob = dnn_keep_prob
                    
                    last_layer = tf.nn.dropout(last_layer, layer_keep_prob)
#                     print('{}_dropout: {}'.format(layer_name, last_layer.get_shape()))
...
#         print('logit: {}'.format(logits.get_shape()))
        ...
        # Multi-task learning - add density prediction head
        if enable_multi_task:
            with tf.variable_scope('density_prediction') as scope:
                # Density prediction predicts number of steps in next few frames
                density_w = tf.get_variable('W', shape=[dnn_output_size, 1], 
                                          initializer=tf.truncated_normal_initializer(stddev=1.0 / dnn_output_size, dtype=dtype),
                                          dtype=dtype)
                density_b = tf.get_variable('b', shape=[1], 
                                          initializer=tf.constant_initializer(0.0), 
                                          dtype=dtype)
            # Linear output for regression task (predicting a count)
            density_logits = tf.squeeze(tf.nn.bias_add(tf.matmul(dnn_output, density_w), density_b), squeeze_dims=[1])
            density_prediction = tf.nn.relu(density_logits)  # Make sure density is non-negative
            density_prediction_inspect = tf.reshape(density_prediction, [batch_size, rnn_nunroll])
            density_prediction_final = tf.squeeze(tf.slice(density_prediction_inspect, [0, rnn_nunroll - 1], [-1, 1]), squeeze_dims=[1])
        ...
        if mode != 'gen':
            ...
            # Multi-task loss
            if enable_multi_task:
                # For density prediction, calculate the density targets from onset targets
                # Count steps in a window ahead
                density_window = 10  # Number of frames to look ahead for density
                
                # Density targets placeholder - will be fed during training
                density_targets = tf.placeholder(dtype, shape=[batch_size * rnn_nunroll], name='density_targets')
                density_target_weights = tf.ones_like(density_targets, dtype=dtype)
                
                # Mean squared error for density prediction (regression task)
                density_errors = tf.square(density_prediction - density_targets)
                avg_density_error = tf.reduce_mean(density_errors)
                
                # Use task weights (defaulting to 0.7 for onset, 0.3 for density if not specified)
                onset_weight = 0.7 if task_weights is None else task_weights[0]
                density_weight = 0.3 if task_weights is None else task_weights[1]
                
                # Combined loss - onset classification and density regression
                avg_neg_log_lhood = onset_weight * avg_neg_log_lhood + density_weight * avg_density_error
                density_errors_inspect = tf.reshape(density_errors, [batch_size, rnn_nunroll])
        ...
        # Export multi-task related tensors if enabled
        if enable_multi_task:
            self.density_prediction = density_prediction_inspect
            self.density_prediction_final = density_prediction_final
        ...
        if mode != 'gen':
            ...
            # Export multi-task related loss tensors if enabled
            if enable_multi_task:
                self.density_errors = density_errors_inspect
                self.density_targets = density_targets
                self.avg_density_error = avg_density_error
